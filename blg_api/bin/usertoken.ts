...
function makeToken(time_hr: number, user_uid: string) {
    ...
    const cipher = crypto.createCipheriv('aes-192-cbc', Buffer.from(process.env.USERTOK_ENC_KEY, 'utf8'), iv);
...
function parseToken(user_enctoken: tokendata_encrypted) {
    try {
        ...
        const decipher = crypto.createDecipheriv('aes-192-cbc', Buffer.from(process.env.USERTOK_ENC_KEY, 'utf8'), iv);
